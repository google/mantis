...
1.  **Sub-Agent Orchestration Loop:** For each iteration of the review loop,
    ...
    -   **Stage 13 (Reflect):** Call the `@mantis-reflect` subagent to parse the
        ...
        must pass the absolute file paths to the execution log files (e.g.
        ...
        threat-model, plan, researcher, dedupe, review, critic, reproduce,
        chain, patch, calibrate) to the `@mantis-reflect` subagent. Resolve
        these paths based on the active coding agent framework's directory
        structure (for example, in Antigravity, logs are located under
    ...
    -   **Stage 15 (Archive & KB Verification):**
        ...
        5.  If `workspace/findings/` exists and contains findings:
            ...
            -   Move all `.json` files and the `.trash/` directory (if it
                ...
                workspace/findings/*.json workspace/archive/findings_pass_N/`).
        6.  If the findings directory is empty or does not exist, ensure it
            exists and skip archiving.
        7.  Increment `N` by 1 to get the new pass number `N_new = N + 1`.
        8.  Write the new pass number `N_new` and the current ISO 8601 timestamp
            to `"pass_number"` and `"last_updated"` in
            `workspace/.mantis_state.json`.
