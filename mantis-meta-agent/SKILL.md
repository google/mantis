...
## name: mantis-meta-agent description: >- Acts as the persistent supervisor, launching and monitoring the automated review campaign. Use when running a long-running, continuous security review campaign that needs autonomous coordination. Don't use for executing individual review stages directly.
...
1. **Sub-Agent Orchestration Loop:** For each iteration of the review loop,
   ...
   - **Initialization / Startup:** Read `N` from `"pass_number"` in
     ...
     **VCS Detection & Recording (VCS Agnostic):** Detect the version control
     ...
     1. Check for Git: run `git rev-parse --is-inside-work-tree`. If this command
        succeeds (returns exit code 0 and output is `true`), a Git repository
        is active. Extract:
        - `commit_hash` (via `git rev-parse HEAD`)
        - `branch` (via `git branch --show-current` or `git rev-parse --abbrev-ref HEAD`)
        - `dirty` status (check if `git status --porcelain` is non-empty)
          Set `vcs_type` to `"git"`.
     1. Check for Mercurial: check if `.hg` directory exists or run `hg root`
        (if Mercurial is installed). If found, extract:
        - `commit_hash` (via `hg id -i`)
        - `branch` (via `hg branch`)
        - `dirty` status (check if `hg status` is non-empty)
          Set `vcs_type` to `"hg"`.
     1. Check for Multi-VCS systems: check if `.repo` directory exists or
        look for other multi-VCS markers. If found, resolve the active manifest
        revision (set `revision`) or branch, and check if any sub-repositories are
        dirty (set `dirty`). Set `vcs_type` to `"multi-vcs"`.
     1. If you confirm that no VCS is active in the repository (e.g., the directory
        is a plain folder with no repository files or metadata), set `vcs_type`
        to `"none"`.
     1. If detection commands fail due to errors, missing tool executables (e.g.
   ...
   - **Stage 13 (Reflect):** Call the `@mantis-reflect` subagent to parse the
     ...
     during this round (every stage that ran: history, summarize, architecture, threat-model, plan, researcher, dedupe, review, critic, reproduce, chain, patch, calibrate) to the `@mantis-reflect`
   ...
   - **Stage 15 (Archive & KB Verification):**
     ...
     1. **Call the `@mantis-architecture` subagent** to perform a final
        ...
        (`workspace/kb/`).
     1. Verify that the KB updates were successfully written and that
        `workspace/learnings.jsonl` was processed.
     1. **Archive and Increment Pass:**
        ...
        - If `workspace/findings/` exists and contains findings:
          ...
          - Move all `.json` files and the `.trash/` directory (if it
            exists) to that archive directory (e.g., `mv workspace/findings/*.json workspace/archive/findings_pass_N/`).
...
1. **Intelligent Supervision & Error Handling:**
...
1. **Monitoring & Reporting:** When the pipeline successfully reproduces a
   ...
   - **Do NOT report findings that failed to reproduce (`repro_status: "failed_to_reproduce"`) as confirmed vulnerabilities.**
...
1. **Human-in-the-Loop Steering & Collaboration:** While you are designed for
...
1. **Resilience & Persistence:** When Stage 15 (Archive & KB Verification)
