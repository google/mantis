...
-   **Command:** `/mantis-reproduce [--reattack] [--finding_id=<uuid>] [--force]
...
1.  **Token-Optimized File Updates:** To minimize LLM output tokens, **do not
    ...
    -   To prevent race conditions during concurrent executions (including
        ...
        *   Atomically replace the target cache file with the temporary file
            (e.g., `os.replace` in Python) to ensure readers never see a
            truncated or incomplete file.
        *   Close the lock file descriptor to release the lock (automatically
    ...
    -   **If run with `--reattack`:** You must append or update the following on
        ...
        -   `"bypassed_patch"`: The new/modified PoC successfully bypassed the
            patch and triggered the bug.
