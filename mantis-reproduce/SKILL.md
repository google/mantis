...
## name: mantis-reproduce description: >- Generates and runs crash reproducers to verify security flaws. Use when viable findings exist and you need to write and execute a script or payload to verify the crash. Don't use for code auditing or patching.
...
- **Command:** `/mantis-reproduce [--reattack] [--finding_id=<uuid>] [--force] [--target_root=<path>] [--state_root=<path>]`
...
1. **Writing and Launching the Reproducer:** Write a self-contained test script
...
1. **Functional & Crash-Aware Validation:** Analyze the output such as stdout,
...
1. **Token-Optimized File Updates:** To minimize LLM output tokens, **do not
   ...
   - Maintain a JSON cache file at
     ...
     directory `state_root/workspace/archive/` exists (e.g., `mkdir -p state_root/workspace/archive/`) before creating, reading, or locking the
     cache file.
   - Key the cache by a stable identifier that persists across loop runs even
     ...
     title and its primary file path: `stable_key = normalized_title + "@" + primary_file_path`.
   ...
   - To prevent race conditions during concurrent executions (including
     ...
     - Perform updates atomically using Python's `fcntl.flock` on this lock
       ...
       1. Increment the integer value for this finding's `stable_key`
          by 1.
       ...
       1. Atomically replace the target cache file with the temporary file
          (e.g., `os.replace` in Python) to ensure readers never see a
          truncated or incomplete file.
       1. Close the lock file descriptor to release the lock
...
1. **Criticism of Reproduction Validity:** To ensure the reproduction is a
