       ...
       2. **Copy for Retry (Bypass Research):** For findings in the archive
          ...
          **copy the finding JSON files directly from the archive (e.g.,
          ...
          - Reproduction previously failed (`"failed_to_reproduce"`) but has had
            **fewer than 2 total reproduction attempts** (check attempt count in
            `workspace/archive/.repro_attempts.json` using the `stable_key`), OR
          - Patch verification failed or was incomplete (`patch_status` is
            `"VERIFICATION_FAILED"`, `"ERROR"`, or `"VERIFICATION_INCOMPLETE"`).
          - *Note:* Do **not** retry/copy findings where `"status"` is
            ...
            or those that have reached the 2-attempt limit in the cache, unless
            the target file has been modified in the current loop (VCS diff
            shows changes).
